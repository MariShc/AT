--ДЗ по sql (база AVIA)
--1.
SELECT ad.MODEL, ad.AIRCRAFT_CODE FROM LANIT.FLIGHTS f 
INNER JOIN LANIT.AIRCRAFTS_DATA ad ON f.AIRCRAFT_CODE = ad.AIRCRAFT_CODE 
...
--15.
SELECT CITY FROM LANIT.AIRPORTS_DATA ad GROUP BY CITY ORDER BY count(AIRPORT_CODE) DESC
...
--17.
SELECT * FROM LANIT.FLIGHTS f WHERE f.DEPARTURE_AIRPORT IN ('SVO','DME','VKO') 
AND TO_CHAR(f.SCHEDULED_ARRIVAL, 'yyyy-mm') = '2017-09'

--18.
SELECT DEPARTURE_AIRPORT, COUNT(FLIGHT_NO) FROM LANIT.FLIGHTS f WHERE DEPARTURE_AIRPORT IN ('SVO','DME','VKO') AND TO_CHAR(SCHEDULED_DEPARTURE, 'yyyy') = '2017' GROUP BY DEPARTURE_AIRPORT

--19.
SELECT DEPARTURE_AIRPORT, TO_CHAR(SCHEDULED_DEPARTURE, 'yyyy-mm'), COUNT(FLIGHT_NO) FROM LANIT.FLIGHTS f WHERE DEPARTURE_AIRPORT IN ('SVO','DME','VKO') AND TO_CHAR(SCHEDULED_DEPARTURE, 'yyyy') = '2017' GROUP BY DEPARTURE_AIRPORT, TO_CHAR(SCHEDULED_DEPARTURE, 'yyyy-mm')
